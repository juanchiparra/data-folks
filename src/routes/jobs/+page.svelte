<script lang="ts">
  ...
  const sortedJobs = $derived(
    [...jobs].sort(
      (a, b) => parseDate(a.expires).getTime() - parseDate(b.expires).getTime(),
    ),
  );
  ...
  const activeJobs = $derived.by(() => {
    ...
    return sortedJobs.filter((job) => parseDate(job.expires).getTime() >= today);
  });
</script>
...
<main>
  ...
  <section class="l-container l-section">
    <p>
      A small board of open positions and freelance gigs for information designers and
      data visualization practitioners. Each listing points to the original offer, so you
      can apply directly with the hiring team.
    </p>
  </section>
